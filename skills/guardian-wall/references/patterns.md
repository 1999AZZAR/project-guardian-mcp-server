...
- **Data Delimiting**: Use complex, random delimiters like `<<<EXTERNAL_UNTRUSTED_CONTENT_ID_RANDOM>>>`, while recognizing that delimiters are not a security boundary.
- **Pre-Processing**: Run `sanitize.py` to normalize text and detect indicators. It does not make content trusted or remove hostile intent.
- **Quarantine**: Do not send suspicious text to another agent with tools or secrets. Report indicators and inspect it only as quoted data in a constrained context.
